...
![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/6f0ab40396b7fc2c15e6f4487d3a0ad7.png)

这种保存写操作命令到日志的持久化方式，就是 Redis 里的 **AOF(Append Only File)** 持久化功能，**注意只会记录写操作命令，读操作命令是不会被记录的**，因为没意义。

在 Redis 中 AOF 持久化功能默认是不开启的，需要我们修改 `redis.conf` 配置文件中的以下参数：

![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/0e2d081af084c41802c7b5de8aa41bd4.png)
...
![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/337021a153944fd0f964ca834e34d0f2.png)
...
而如果先执行写操作命令再记录日志的话，只有在该命令执行成功后，才将命令记录到 AOF 日志里，这样就**不用额外的检查开销，保证记录在 AOF 日志里的命令都是可执行并且正确的**。
...
因为**将命令写入到日志的这个操作也是在主进程完成的（执行命令也是在主进程）**，也就是说这两个操作是同步的。

![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/28afd536c57a46447ddab0a2062abe84.png)
...
![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/4eeef4dd1bedd2ffe0b84d4eaa0dbdea.png)
...
- **No**，意味着不由 Redis 控制写回硬盘的时机，转交给操作系统控制写回的时机，也就是每次写操作命令执行完后，先将命令写入到 AOF 文件的内核缓冲区，再由**操作系统决定何时将缓冲区内容写回硬盘。**

这 3 种写回策略都无法能完美解决**「主进程阻塞」**和**「数据丢失」**的问题，因为两个问题是对立的，偏向于一边的话，就会要牺牲另外一边，原因如下：
...
![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/98987d9417b2bab43087f45fc959d32a.png)
...
![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/f64829ffc2e9e006b090f9aae51035ee.png)
...
> 简单总结：
>
> 1. AOF以追加的方式记录每次写命令，当AOF日志文件大小超过设定的阈值时，会触发重写机制，主进程创建一个子进程`bgrewriteaof`负责将数据库中的所有键值对，转化成写命令追加到一个新的AOF文件中，最后覆盖掉原有文件，压缩文件大小。
> 2. 由于使用子进程执行重写操作，父进程可以正常进行。有两个地方会导致主进程阻塞：创建子进程时需要页表复制；触发重写机制时，父进程修改数据会发生写时复制。通过写时复制实现父子进程数据隔离，此时会复制修改数据的物理内存，修改虚拟内存的指向。该操作由于多线程的加锁机制。
> 3. 重写时父子进程共享物理内容，通过**写时复制解决读写冲突**，但是还存在aof文件和数据库中的数据不一致的问题，通过加一个**AOF重写缓冲区**解决。写命令执行后，会同时追加到AOF缓冲区和重写缓存区。
> 4. 重写完成后，子进程发信号给主进程，主进程会调用处理函数，将重写缓存区的数据追加到重写的aof日志文件中，并覆盖掉旧的aof文件。处理过程主进程会阻塞。
...
![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/723d6c580c05400b3841bc69566dd61b.png)
...
- 子进程带有主进程的数据副本（*数据副本怎么产生的后面会说*），这里使用子进程而不是线程，因为如果是**使用线程**，多线程之间会共享内存，那么在修改共享内存数据的时候，**需要通过加锁来保证数据的安全，而这样就会降低性能**。而使用**子进程**，创建子进程时，父子进程是共享内存数据的，不过这个共享的内存只能以只读的方式，而当父子进程任意一方修改了该共享内存，就会发生**「写时复制」**，于是**父子进程就有了独立的数据副本，就不用加锁来保证数据安全**。

子进程是怎么拥有主进程一样的数据副本的呢？

> 写AOF日志是在主进程的，而AOF重写是使用的子进程`background rewrite aof`

主进程在通过 `fork` 系统调用生成 bgrewriteaof 子进程时，操作系统会把主进程的「**页表**」复制一份给子进程，这个页表记录着虚拟地址和物理地址映射关系，而不会复制物理内存，也就是说，两者的虚拟空间不同，但其对应的物理空间是同一个。

![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/5a1f2a90b5f3821c19bea3b7a5f27fa1.png)
...
![img](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/d4cfac545377b54dd035c775603b4936.png)
...
![在这里插入图片描述](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/202105270918298.png)
