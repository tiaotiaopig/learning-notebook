...
因此**在 Redis 恢复数据时， RDB 恢复数据的效率会比 AOF 高些，因为直接将 RDB 文件读入内存就可以，不需要像 AOF 那样还需要额外执行操作命令的步骤才能恢复数据**。

> 简单总结：
>
> 1. RDB使用的**全量快照**，每次执行`bgsave`都会将数据库中的所有数据保存到磁盘，不能过于频繁。
> 2. 主进程写和子进程读发生读写冲突时，采用**写时复制**解决。RDB数据和数据库真实数据不一致，只能下次RDB时写入。
> 3. Redis4.0时增加了混合持久化的方式。主要修改了aof重写的机制，`bgrewriteaof`子进程先采用**RDB的方式**将与主进程共享的数据**全量**写到新的aof文件中，然后再将aof重写缓存区的数据以**aof方式增量**追加到aof文件之后。充分利用了两种方式的优点。
...
![图片](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/c34a9d1f58d602ff1fe8601f7270baa7.png)
...
![图片](https://raw.githubusercontent.com/tiaotiaopig/feng-images-store/main/images/f67379b60d151262753fec3b817b8617.png)
